...
export default function Friendlist() {
  ...
  const handleInvitationSubmit = async () => {
    ...
    const response = await send_request("/request_friend", {
      user_id: userId,
      name: username,
    });
  ...
  const confirmRequest = async (friend, choice) => {
    const response = await send_request("/confirm_friend", {
      user_id: userId,
      name: friend,
      confirm: choice,
    });
    if (response.error != undefined) {
      ...
      if (
        response.error ==
        "account not found, please send the friend request again"
      ) {
  ...
  const handleDeleteClick = async () => {
    for (const friend of selectedFriends) {
      const response = await send_request("/delete_friend", {
        user_id: userId,
        name: friend,
      });
      if (response.error !== undefined) {
        if (
          ...
          response.error === "friend not found: 1"
        ) {
  ...
  const handleLastMessage = async () => {
    const updatedFriendList = await Promise.all(
      friendList.map(async (friend) => {
        ...
        const response = await send_request("/get_messages", {
          user_id: userId,
          name: fname,
    ...
    <ThemeProvider theme={theme}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
        ...
        >
          ...
          <DialogContent>
            <DialogContentText>{languageData.instruction}</DialogContentText>
            <Autocomplete
              {...Props}
              id="username"
          ...
          <DialogActions>
            <Button onClick={handleInvitationSubmit}>
              {languageData.sendInvitation}
            </Button>
            <Button onClick={handleInvitationClose}>
              {languageData.cancel}
            </Button>
          </DialogActions>
        </Dialog>
        ...
        <Fab aria-label="friend request" color="primary" size="small">
          ...
          <DialogContent>
            <List sx={{ width: 500 }}>
              {requestList.length > 0 ? (
                requestList.map(({ name, confirm, chatroom }, index) => (
                  <ListItem key={name + index}>
                    <ListItemText
                      primary={`${name}` + languageData.wantToAdd}
                    />
                    <ListItemSecondaryAction>
                      <IconButton
                        ...
                        aria-label="accept"
                        sx={{ color: "green" }}
                        onClick={() => confirmRequest(name, true)}
                      >
                        <CheckIcon />
                      </IconButton>
                      <IconButton
                        ...
                        onClick={() => confirmRequest(name, false)}
                      >
                        <CloseIcon />
                      </IconButton>
                    </ListItemSecondaryAction>
      ...
      <Box sx={{ pb: 7 }} ref={ref}>
        ...
        <List sx={{ width: "70%" }}>
          {friendList.length > 0 ? (
            friendList.map(({ name, confirm, message, avatar_color }, id) => (
              <ListItemButton key={id + name}>
                {deleteMode && (
                  <ListItemIcon>
                    <Checkbox
                      onChange={(e) =>
                        handleFriendCheckboxChange(name, e.target.checked)
                      }
